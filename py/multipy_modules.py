...
#    [lat1,lon1]=dm2dd(float(dfcsv['lat'][0]),float(dfcsv['lon'][0]))
    ...
    return float(clim[idlon][idlat])

def get_depth_bathy(loni,lati,mindist_allowed=10):
    """input lon lat
    return the bottom depth in ocean"""
...
def get_gomofs_url(date):
    """
    input the date and time,the format of date is:datetime.datetime(2019, 2, 27, 11, 56, 51, 666857)
    return the url address about gomofs file.
    """
...
def get_gomofs_url_forcast(date,forcastdate=True):
    """
    fuction: get the gomofs forcast file url address through the date and time.
    input: date(date and time), the format of date is GMT TIME:datetime.datetime(2019, 2, 27, 11, 56, 51, 666857)
        forcastdate equal date or True
...
def get_gomofs(dtime,latp,lonp,depth='bottom',mindistance=20,autocheck=True,fortype='temperature'):
    """
    input:
    dime:the format dtime(GMT) is: datetime.datetime(2019, 2, 27, 11, 56, 51, 666857)
    lat and lon: lat and lon use decimal degrees
    depth:if the depth is under the water, please must add the marker of '-'
    default parameter:
    mindistance:the unit is mile of distance(the mindistance is the minimum threshold between input point and model point )
    fortype:'temperature','tempdepth'
    return the temperature (or temperature and depth) of specify location
...
def get_FVCOM_temp(latp,lonp,dtime,depth,mindistance=2,fortype='temperature'): # gets modeled temp using GOM3 forecast 
    ''' 
    function:
...
#        point_h=depth
    if fortype=='temperature':
        ...
        return 'please input right fortype'
